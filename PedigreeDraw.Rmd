...
```{r}
library(randomNames)
test <-randomNames(20,which.names="first",gender=1,ethnicity=4)
test
```
...
```{r}
dat.ad <-read.csv("./scratch/AutoD.csv")
dat.ad
```
Now the pedigree.
```{r}
ped.ad <-with(dat.ad,pedigree(id,dadid=fatherid,momid=motherid,sex=sex,affected=aff,missid="0"))
plot(ped.ad,cex=.80)
```
